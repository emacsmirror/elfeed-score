...
#+BEGIN_SRC elisp
(package-install-file "elfeed-score-1.2.11.tar")
#+END_SRC
...
#+BEGIN_SRC bash
cd /tmp
curl -L --output elfeed-score-1.2.11.tar.gz https://github.com/sp1ff/elfeed-score/releases/download/1.2.11/elfeed-score-1.2.11.tar.gz
tar xvf elfeed-score-1.2.11.tar.gz && cd elfeed-score-1.2.11
...
curl -L --output=elfeed-score-1.2.11.tar.gz https://github.com/sp1ff/elfeed-score/releases/download/1.2.11/elfeed-score-1.2.11.tar.gz
tar xvf elfeed-score-1.2.11.tar.gz && cd elfeed-score-1.2.11
export EMACSLOADPATH=/tmp/elfeed-score-1.2.11:/tmp/elfeed:/tmp/elfeed/tests:$EMACSLOADPATH
./configure
make
make check
sudo make install
...
Again, unless you already use =EMACSLOADPATH=, you'll need to set =EMACSLOADPATH= appropriately to your system, something like =EMACSLOADPATH=/tmp/elfeed-score-1.2.11:/tmp/elfeed:/tmp/elfeed/tests:/usr/share/emacs/25.2/lisp=.
